...
export class GoogleAPI {
    static async search(query: string): Promise<GoogleAPIDataInterface | undefined> {
        try {
            const response1 = await axios.get(`https://content-customsearch.googleapis.com/customsearch/v1?cx=9625fbdec74e34747&q=${query}&cr=countryES&filter=1&exactTerms=funko&gl=es&hl=es&key=${process.env.GOOGLE_API_KEY}`)
            const response2 = await axios.get(`https://content-customsearch.googleapis.com/customsearch/v1?start=11&cx=9625fbdec74e34747&q=${query}&cr=countryES&lr=lang_es&filter=1&exactTerms=funko&gl=es&hl=es&key=${process.env.GOOGLE_API_KEY}`)
